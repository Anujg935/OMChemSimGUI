...
class DistCol():
    ...
    def OM_Flowsheet_Eqn(self, addedcomp):
        ...
        self.OM_data_eqn = self.OM_data_eqn + ('connect('+self.name+'.'+condensor_duty+','+ self.EngStm1+'.inlet);\n')
        self.OM_data_eqn = self.OM_data_eqn + ('connect('+self.name+'.reboiler_duty'+', '+self.EngStm2+'.inlet);\n')
        ...
        self.OM_data_eqn = self.OM_data_eqn + (self.OutputStms[1].name+'.'+'totMolFlow[1] = '+self.OutputStms[1].Prop['totMolFlo[1]']+';\n')
        if self.mode=="refluxRatio":
            self.OM_data_eqn = self.OM_data_eqn + (self.name+'.'+self.mode+'='+ self.modeVal + ';\n')
        else:
            self.OM_data_eqn = self.OM_data_eqn + (self.name+'.condensor.'+self.mode+'='+ self.modeVal + ';\n')
