package Simulator
  ...
  package Files
    ...
    package Thermodynamic_Functions
      ...
      algorithm
        for i in 1:n - 1 loop
          Cp[i] := VapCpId(VapCp, 298.15 + i * (T - 298.15) / n, Tc);
        end for;
        if T > 298.15 then
          Ent := (T - 298.15) * (VapCpId(VapCp, T, Tc) / 2 + sum(Cp[:]) + VapCpId(VapCp, 298.15, Tc) / 2) / n;
        else
          Ent := -(T - 298.15) * (VapCpId(VapCp, T, Tc) / 2 + sum(Cp[:]) + VapCpId(VapCp, 298.15, Tc) / 2) / n;
        end if;
      end HVapId;
      ...
      algorithm
        Entr := 0;
        for i in 1:n - 1 loop
          Cp[i] := Simulator.Files.Thermodynamic_Functions.VapCpId(VapCp, 298.15 + i * (T - 298.15) / n, Tc) / (298.15 + i * (T - 298.15) / n);
        end for;
        if T >= Tref then
          Entr := (T - 298.15) * (Simulator.Files.Thermodynamic_Functions.VapCpId(VapCp, T, Tc) / (2 * T) + sum(Cp[:]) + Simulator.Files.Thermodynamic_Functions.VapCpId(VapCp, 298.15, Tc) / (2 * 298.15)) / n;
        else
          Entr := -(T - 298.15) * (Simulator.Files.Thermodynamic_Functions.VapCpId(VapCp, T, Tc) / (2 * T) + sum(Cp[:]) + Simulator.Files.Thermodynamic_Functions.VapCpId(VapCp, 298.15, Tc) / (2 * 298.15)) / n;
        end if;
        if x > 0 and y > 0 then
...
//Energy Balance
      for i in 1:NOC loop
//Specific Heat and Enthalpy calculation
        compMolSpHeat[2, i] = Thermodynamic_Functions.LiqCpId(comp[i].LiqCp, comp[i].VapCp, T, comp[i].Tc);
        compMolSpHeat[3, i] = Thermodynamic_Functions.VapCpId(comp[i].VapCp, T, comp[i].Tc);
        compMolEnth[2, i] = Thermodynamic_Functions.HLiqId(comp[i].SH, comp[i].VapCp, comp[i].HOV, comp[i].Tc, T);
        compMolEnth[3, i] = Thermodynamic_Functions.HVapId(comp[i].SH, comp[i].VapCp, comp[i].HOV, comp[i].Tc, T);
        (compMolEntr[2, i], compMolEntr[3, i]) = Thermodynamic_Functions.SId(comp[i].AS, comp[i].VapCp, comp[i].HOV, comp[i].Tb, comp[i].Tc, T, P, compMolFrac[2, i], compMolFrac[3, i]);
      end for;
...
model Mat_Stm_RL
  ...
  package Unit_Operations
    ...
    model Mixer
      import Simulator.Files.*;
      parameter Integer NOC "Number of Components", NI = 2 "Number of Input streams";
      parameter Chemsep_Database.General_Properties comp[NOC];
...
//vap pressure of mixture at outT
    ...
    equation
      inlet.T = outlet.T;
      inlet.P = outlet.P;
      inlet.mixMolFrac = outlet.mixMolFrac;
      inlet.mixMolFlo = outlet.mixMolFlo;
      inlet.mixMolEnth = outlet.mixMolEnth;
      inlet.mixMolEntr = outlet.mixMolEntr;
      inlet.vapPhasMolFrac = outlet.vapPhasMolFrac;
    end Recycle;
...
//sum y = 1
    ...
    model ConversionReactor
      //The following parameters are inputs to the reactor. The inlet conditions for connected inlet material stream should be defined in the test model.
      parameter Integer Nr = 1 "No. of reactions";
      parameter Integer b = 1 "Base component of a reaction";
      Real X "Conversion of base component";
      parameter Integer Z = 3 "No. of reaction components(reactants + products)";
      parameter Real a[Z] "Reaction coefficients";
      parameter Real Pdrop = 0 "pressure drop";
      parameter Real Tdef = 300 "Defined outlet temperature, only applicable if option 2 is chosen";
      parameter Simulator.Files.Chemsep_Database.General_Properties comp[Z];
      parameter Integer operation "1-isothermal,2-define outlet temperature,3-adiabatic; choose the required operation";
      Real n[Z] "Inlet component wise molar flowrate";
      Real mixMolFracIn[Z](each min = 0, max = 1) "mixture mole fraction";
      Real N[Z] "No. of moles of each component (unit=mol)";
      Real mixMolFracOut[Z](each min = 0, max = 1) "Mixture Mole fraction of each component";
      Real inMixMolEntr "inlet mixture molar entropy";
      Real outMixMolEntr "outlet mixture molar entropy";
      Real inMixMolEnth "inlet mixture molar enthalpy";
      Real outMixMolEnth "outlet mixture molar enthalpy";
      //Integer d"dummy variable for defining base component in equations";
      Real inMolFlo "inlet mixture molar flow rate";
      Real outMolFlo "outlet mixture molar flow rate";
      Real inP "Inlet pressure";
      Real outP "Outlet pressure";
      Real inT "Inlet Temperature";
      Real outT "Outlet Temperature";
      //Real stcheck[Nr];
      Real inVapPhasMolFrac "Inlet Vapor Mole Fraction";
      Real outVapPhasMolFrac "Outlet Vapor Mole Fraction";
      Simulator.Files.Connection.matConn inlet(connNOC = Z) annotation(
        Placement(visible = true, transformation(origin = {-104, 6}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-70, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Simulator.Files.Connection.matConn outlet(connNOC = Z) annotation(
        Placement(visible = true, transformation(origin = {104, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {70, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Simulator.Files.Connection.enConn EnergyStream annotation(
        Placement(visible = true, transformation(origin = {2, -96}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {2, -110}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    equation
//stcheck=stoichiometrycheck(Nr,Z,comp.MW,a);
//To check the stoichiometry of the reaction, we use stoichiometry check function.
//  for h in 1:Nr loop
//    assert(stcheck == 1, "The stoichiometry is not correct");
//  end for;
//connector equations
      inlet.P = inP;
      inlet.T = inT;
      ...
      outlet.mixMolEnth = outMixMolEnth;
      outlet.mixMolEntr = outMixMolEntr;
      outlet.mixMolFrac[:] = mixMolFracOut[:];
      outlet.vapPhasMolFrac = outVapPhasMolFrac;
//Reactor equations
      for r in 1:Z loop
        n[r] = inMolFlo * mixMolFracIn[r];
      end for;
      for i in 1:Z loop
        N[i] = n[i] - X * n[b] * (a[i] / a[b]);
      end for;
//  if Nr > 1 then
//    for k in 2:Nr loop
//      d[k] = b[k];
//      for p in 1:Z loop
//        N[p,k] = N[p,k - 1] - a[p,k] / a[d[k], k] * n[d[k]] * X[k];
//      end for;
//    end for;
//  end if;
//  for g in 1:Z loop
//    assert(N[g, Nr] >= 0, "Conversion more than 1 for this component");
//  end for;
      outMolFlo = sum(N[1:Z]);
      for j in 1:Z loop
        mixMolFracOut[j] = N[j] / outMolFlo;
      end for;
//operation equations
      if operation == 1 then
        inP - Pdrop = outP;
        inT = outT;
        EnergyStream.enFlo = outMixMolEnth * outMolFlo - inMixMolEnth * inMolFlo;
      elseif operation == 3 then
        inP - Pdrop = outP;
        outMixMolEnth * outMolFlo = inMixMolEnth * inMolFlo;
        EnergyStream.enFlo = 0;
      else
        inP - Pdrop = outP;
        outT = Tdef;
        EnergyStream.enFlo = outMixMolEnth * outMolFlo - inMixMolEnth * inMolFlo;
      ...
      annotation(
        Icon(coordinateSystem(extent = {{-60, -100}, {60, 100}}, initialScale = 0.1), graphics = {Polygon(origin = {0, -0.999844}, points = {{-40, 101}, {-60, 80.9998}, {-60, -79.0002}, {-40, -99}, {40, -99.0002}, {60, -79.0002}, {60, 80.9998}, {40, 101}, {-40, 101}}), Bitmap(extent = {{2, 88}, {2, 88}}), Line(origin = {0, 80}, points = {{-60, 0}, {60, 0}, {58, 0}}), Line(origin = {0, -80}, points = {{60, 0}, {-60, 0}}), Line(points = {{-60, 80}, {60, -80}}), Line(points = {{-60, -80}, {60, 80}})}),
